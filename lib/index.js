...
var MtSvgLines = (function(_React$Component) {
	...
	function MtSvgLines(props) {
		...
		var _this = _possibleConstructorReturn(
			this,
			(MtSvgLines.__proto__ || Object.getPrototypeOf(MtSvgLines)).call(
				this,
				props
			)
		);
		...
		_this.state = {
			classKey: 'mt-' + _utils.shortUID(), // unique class name for the wrapper, an internal "trigger" (re-gen each time anim is to run)
			css: '', // generated CSS
		...
		},
		{
			...
			value: function render() {
				...
				return _react2.default.createElement(
					...
					_react2.default.createElement('style', null, css),
					children
				);
			},
     ...
     */
		},
		{
			...
			value: function animate() {
				...
				var isStartNewAnim =
					animate !== false && classKey !== this._lastClassKey;
				...
				if (isStartNewAnim) {
					...
					if (isAnimJS) {
						...
						var tween = new _tween2.default.Tween(this._tweenData)
						...
						var staggerMult =
							(0, _utils.clamp)(stagger, 0, 100) / 100; // convert pct to 0-1
						var pathStaggerDelay =
							stagger > 0
								? (duration / pathQty) * staggerMult
								: 0;
						var pathDrawDuration =
							stagger > 0
								? duration -
								  pathStaggerDelay *
										(pathQty - 1) *
			...
			value: function getPathData(pathElems) {
				...
				[].forEach.call(pathElems, function(pathEl, i) {
					if (!_this4.hasSkipAttr(pathEl.attributes)) {
						var pathLengh = (0, _utils.trimFloat)(
							pathEl.getTotalLength() || 0
						);
						pathData.from[i] = 0;
						pathData.to[i] = -pathLengh;
					}
				});
		...
		},
		{
			...
			value: function setStrokeDasharray(pathElems, pathData) {
				[].forEach.call(pathElems, function(pathEl, i) {
					...
					pathEl.style.strokeDasharray = pathLengh;
				});
			},
		...
		},
		{
			...
			value: function getPathLengths() {
				...
				return [].map.call(pathElems, function(pathEl) {
					return _this5.hasSkipAttr(pathEl.attributes)
						? 0
						: (0, _utils.trimFloat)(pathEl.getTotalLength() || 0);
				});
			},
     ...
     */
		},
		{
			...
			value: function getPathCss(
				...
				var totalDelay = length
					? (0, _utils.trimFloat)(
							(startDelay + staggerDelay * index) / 1000
					  )
					: 0;
				var startOpacity = fade ? 0.01 : 1;

				duration = duration
					? (0, _utils.trimFloat)(duration / 1000)
					: 0;
...
MtSvgLines.propTypes = {
	...
	timing: _propTypes2.default.oneOf([
		'ease',
		'ease-in',
		'ease-out',
		'ease-in-out',
