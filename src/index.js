...
export default class MtSvgLines extends React.Component {
   ...
   */
	selectPathElems() {
		const svgEl =
			this._svgWrapper && this._svgWrapper.getElementsByTagName('svg')[0];
   ...
   */
	getPathLengths() {
		...
		return [].map.call(
			pathElems,
			pathEl =>
				this.hasSkipAttr(pathEl.attributes)
					? 0
					: trimFloat(pathEl.getTotalLength() || 0)
		);
	}
   ...
   */
	getPathCss(index, length, startDelay, staggerDelay, duration) {
		...
		const totalDelay = length
			? trimFloat((startDelay + staggerDelay * index) / 1000)
			: 0;
	...
	animate() {
		...
		if (isStartNewAnim) {
			...
			if (isAnimJS) {
				...
				const pathStaggerDelay =
					stagger > 0 ? (duration / pathQty) * staggerMult : 0;
				const pathDrawDuration =
					stagger > 0
						? duration -
						  pathStaggerDelay * (pathQty - 1) * (2 - staggerMult)
						: duration;
